...
首次运行会从 BWiki 拉取地图元数据、点位、文本图层，以及缩放级别 z=4 的瓦片（最低细节、覆盖整图）。这一步需要网络。完成后所有内容存到 `<exe 同级目录>/cache/`，后续启动直接走本地。更高缩放的瓦片在你放大查看具体区域时按需下载。或者在设置中手动下载。（开启导航功能时也会自动下载）
...
1. 打开设置界面点"框选小地图"（默认热键 `Ctrl+Alt+R`）。软件会对屏幕进行截图，在软件内完整框选小地图内边框即可。
2. 在地图上**右键玩家所在的真实位置**，菜单选"玩家在这里"。这一步会以你右键的坐标为真值在 K ∈ [0.5, 1.0] 范围扫描最佳缩放系数 K，写到 `data/calibration.json`，下次启动自动加载。
3. 在设置中点"开始追踪"或创建好路径后点击开始导航。每 ~600 ms 截一次小地图，识别出的位置 + 朝向叠到大地图上。
4. 选中一条路径点"开始导航"，匹配频率提升到 ~400 ms。路径会按当前进度自动分成已走 / 未走，自动居中可在设置里改成"始终居中 / 仅导航时居中 / 关闭"。
5. 玩家偏离路径时投影锁在当前段，不会跨段串到后续段。完成当前段后锁自动前进。
...
**首次抓取失败 / 卡住**  
检查能否访问 `wiki.biligame.com` 。再次启动会从断点续抓；用 `-reset` 强制全量重抓。
